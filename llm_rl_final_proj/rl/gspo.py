    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                # GSPO: aggregate token log-ratios into one sequence-level ratio before clipping
                seq_ratio = torch.exp(masked_mean_per_row(new_logp - mb.old_logprobs, mask))  # [B]
                unclipped = seq_ratio * adv
                clipped = torch.clamp(seq_ratio, 1 - cfg.clip_eps, 1 + cfg.clip_eps) * adv
