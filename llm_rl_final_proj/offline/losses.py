...
def compute_offline_preference_loss(
    *,
    ...
    if algo == "dpo":
        ...
        chosen_rewards = torch.sort(policy_scores.chosen_logp_sum - reference_scores.chosen_logp_sum, dim=0).values
        rejected_rewards = torch.sort(policy_scores.rejected_logp_sum - reference_scores.rejected_logp_sum, dim=0).values
        quantile_gap = chosen_rewards - rejected_rewards
        losses = torch.mean(-F.logsigmoid(beta * quantile_gap))
        metrics.update(
            {
